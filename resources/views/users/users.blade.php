<x-layout>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row" style="margin-bottom: 10px;">
                <div class="col">
                    <a href="{{ route('userAddPage') }}"><button type="button"
                                ...
                                <td class="text-center">
                                    @foreach ($user->roles as $role)
                                        <span class="btn btn-info btn-xs"
                                            style="border-radius: 5">{{ $role->name }}</span><br>
                                    @endforeach
                                </td>
                                <td class="text-center">
                                    <a href="{{ route('userUpdatePage', $user->id) }}"><button
    ...
    @section('script')
        ...
        @if (session()->has('Add'))
            <script>
                swal("User Add", "{!! session()->get('Add') !!}", "success", {})

            </script>
        @endif
        @if (session()->has('Update'))
            <script>
                swal("User Update", "{!! session()->get('Update') !!}", "success", {})
